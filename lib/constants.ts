...
export const FEATURED_PROJECTS = [
  {
    ...
    impact: ['110% search satisfaction improvement (42% → 90%)', '91% recall@10 achieved', '99.3% latency reduction (6s → 40ms P99)', '€420K annual savings (18% fewer support calls)', '9 European languages supported', 'Continuous quality benchmarking across production, UAT, and Dev'],
    tags: ['Semantic Search', 'RAG','Hybrid Retrieval', 'Vector Search', 'E5 Large', 'GPT-4', 'LlamaIndex','Databricks', 'Unity Catalog'],
    category: 'flagship',
    featured: true,
    businessProblem:
    ...
    architecture: [
      ...
      'Delta Change Data Feed synchronizes model- and content-type-specific indexes into Databricks Vector Search.',
      'A secured model-serving endpoint exposes hybrid retrieval through a REST API gateway connecting the web application to inference services.',
      'LLM summarization for high-confidence answer generation',
    ],
    technicalDecisions: [
      'Combined BM25 lexical precision for exact flight and booking terms with dense vector embeddings and reranking for semantic intent.',
      'Benchmarked Azure OpenAI text-embedding-3-large against HuggingFace intfloat/multilingual-e5-large-instruct and selected E5 Large with hybrid ranking for multilingual quality, cost efficiency, and serving latency.',
      'Used decoupled governed index tables partitioned by model and content type to support zero-downtime model upgrades.',
      'Kept retrieval deterministic and used GPT-4 only for final summarization to control cost and hallucinations.',
    ],
    productionChallenges: [
      'Used incremental snapshotting to detect content updates and deletions, purge obsolete pages, and prevent broken links from being served.',
      'Scheduled weekly drift analysis against a gold-standard dataset, with automated alerts when average recall falls below the 65% baseline threshold.',
      'Built a Databricks App with Streamlit for batch benchmarking across Recall, Precision, F1, P@K, and NDCG@K, complemented by Databricks Genie AI Agents for conversational production insights.',
      'Optimized endpoint cold starts with lazy model loading, standardized 100–500 word chunk sizing, and query-time deduplication.',
    ...
    outcomes: [
      'Achieved approximately 80.5% top-10 recall and 66.66% top-5 recall on complex customer queries using the E5 Large hybrid pipeline.',
      'Expanded from an English MVP to nine European languages: English, German, Czech, Austrian German, Italian, French, Spanish, Swedish, and Dutch.',
      'Established a governed AI search foundation with Delta Lake, Vector Search, CI/CD deployment bundles, continuous benchmarking, and observability.',
      'Created a maintainable content lifecycle that refreshes changed pages and removes deleted or unavailable content automatically.',
